...
export function CreateDepartmentDialog({
  ...
  const handleSubmit = async (data: DepartmentFormData) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json.error?.message || json.message || 'Không thể tạo phòng ban.');
      }

      onSuccess(json.message || 'Tạo phòng ban thành công.');
      onClose();
      router.refresh();
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
      ...
      >
        <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
          <h2 id="create-dialog-title" className="text-base font-bold text-white">
            Thêm phòng ban
          </h2>
          <button
            type="button"
